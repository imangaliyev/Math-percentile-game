...
export default function App() {
  ...
  useEffect(() => {
    // Fetch initial scores when the component mounts
    const fetchScores = async () => {
      try {
        ...
        console.error("Failed to fetch scores:", error.message, error.stack);
        setError("Could not connect to the database to fetch scores. Please try again later.");
        setGameState(GameState.Welcome); // Still go to welcome, but with an error.
      }
    };
    fetchScores();
  }, []);
  ...
  const handleQuizComplete = useCallback(async (score: number) => {
    ...
    try {
      ...
      // Fetch the updated list of scores to include the latest one.
      const updatedScores = await getAllScores();
      setAllScores(updatedScores);
    } catch (err) {
      const error = err as Error;
      console.error("Failed to save score:", error.message, error.stack);
      // Even if saving fails, show the user their result with the scores we already have.
  ...
  const renderContent = () => {
    switch (gameState) {
      ...
      case GameState.Loading:
        return <Spinner message={questions.length > 0 ? "Starting quiz..." : "Loading historical scores..."} />;
      case GameState.Quiz:
